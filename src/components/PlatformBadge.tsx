...
const PlatformBadge = forwardRef<HTMLDivElement, PlatformBadgeProps>(
  ({ link, className = "" }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={`absolute bottom-3 right-3 w-10 h-10 rounded-full bg-card border-2 border-foreground/80 flex items-center justify-center opacity-70 group-hover:opacity-100 transition-opacity duration-300 shadow-pop ${className}`}
      >
        {platform === "telegram" && (
        ...
        {platform === "drive" && (
          <svg viewBox="0 0 87.3 78" className="w-5 h-5 text-foreground" fill="currentColor">
