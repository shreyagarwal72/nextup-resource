...
import { Palette, ExternalLink, SearchX, ChevronDown, Github } from "lucide-react";
import { useDesignMd, type DesignMdEntry } from "@/hooks/useDesignMd";
...
const DesignMdCard = ({ item, accent }: { item: DesignMdEntry; accent: Accent }) => {
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {shown.map((item, i) => (
          <DesignMdCard key={`${item.name}-${i}`} item={item} accent={accent} />
        ))}
      </div>
...
const DesignMd = () => {
  const { loading, error, data } = useDesignMd();
  ...
  useEffect(() => {
    document.title = "DESIGN.md — Brand Design System Catalog";
  }, []);
  ...
  const filtered = useMemo(() => {
    if (!data) return {} as Record<string, DesignMdEntry[]>;
    const q = debounced.trim().toLowerCase();
    const result: Record<string, DesignMdEntry[]> = {};
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center max-w-3xl mx-auto animate-fade-in">
              ...
              <p className="text-lg text-muted-foreground mt-5">
                                A curated catalog of brand DESIGN.md files, ready to drop into a project so AI coding agents can generate matching UI — sourced from the community-maintained{" "}
                <a
              ...
              <div className="max-w-3xl mx-auto mt-5 p-4 bg-card border-2 border-foreground/80 rounded-2xl shadow-pop text-center">
                <p className="text-sm font-bold text-foreground font-heading flex items-center justify-center gap-1.5 flex-wrap">
                  ...
                  — a community-maintained list of brand DESIGN.md files for AI-driven UI generation. ❤️
                </p>
              </div>
              ...
              <div className="mt-7 max-w-xl mx-auto">
                <SearchBox
                  ...
                  ariaLabel="Search DESIGN.md brand design systems"
                />
              </div>
              ...
              {!loading && !error && (
                <div className="mt-4 inline-flex items-center gap-2 px-3 py-1 rounded-full border-2 border-foreground/30 text-xs font-bold text-muted-foreground">
                  ...
                  {filteredCount} of {totalCount} design systems
                </div>
              )}
            </div>
          </div>
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl">
            {loading && (
              <div className="max-w-md mx-auto text-center pop-card p-8">
                ...
                <p className="text-sm text-muted-foreground">Loading the DESIGN.md catalog…</p>
              </div>
            )}
            ...
            {error && (
              <div className="max-w-md mx-auto text-center pop-card p-8">
                ...
                <p className="text-sm text-muted-foreground">
                  The DESIGN.md list couldn't be fetched right now. Check your connection and try again shortly.
                </p>
              </div>
            )}
            ...
            {!loading && !error && Object.keys(filtered).length === 0 && (
              <div className="max-w-md mx-auto text-center pop-card p-8">
                ...
                <h3 className="font-heading font-bold text-xl mb-2">No design systems found</h3>
