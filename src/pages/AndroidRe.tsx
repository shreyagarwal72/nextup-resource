...
import { Bug, ExternalLink, SearchX, ChevronDown, Github } from "lucide-react";
import { useAndroidRe, type AndroidReEntry } from "@/hooks/useAndroidRe";
...
const AndroidReCard = ({ item, accent }: { item: AndroidReEntry; accent: Accent }) => {
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {shown.map((item, i) => (
          <AndroidReCard key={`${item.name}-${i}`} item={item} accent={accent} />
        ))}
      </div>
...
const AndroidRe = () => {
  const { loading, error, data } = useAndroidRe();
  ...
  useEffect(() => {
    document.title = "Android RE — Reverse Engineering Training & Tools";
  }, []);
  ...
  const filtered = useMemo(() => {
    if (!data) return {} as Record<string, AndroidReEntry[]>;
    const q = debounced.trim().toLowerCase();
    const result: Record<string, AndroidReEntry[]> = {};
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center max-w-3xl mx-auto animate-fade-in">
              ...
              <p className="text-lg text-muted-foreground mt-5">
                                A curated catalog of Android reverse engineering training, tools and resources — sourced from the community-maintained{" "}
                <a
                  href="https://github.com/user1342/Awesome-Android-Reverse-Engineering"
                  target="_blank"
                  rel="noopener noreferrer"
              ...
              <div className="max-w-3xl mx-auto mt-5 p-4 bg-card border-2 border-foreground/80 rounded-2xl shadow-pop text-center">
                <p className="text-sm font-bold text-foreground font-heading flex items-center justify-center gap-1.5 flex-wrap">
                  ...
                  — a community-maintained list of Android reverse engineering training, tools and resources. ❤️
                </p>
              </div>
              ...
              <div className="mt-7 max-w-xl mx-auto">
                <SearchBox
                  value={query}
                  onChange={setQuery}
                  placeholder="Search tools, courses, books..."
                  ariaLabel="Search Android reverse engineering resources"
                />
              </div>
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl">
            {loading && (
              <div className="max-w-md mx-auto text-center pop-card p-8">
                ...
                <p className="text-sm text-muted-foreground">Loading the Android RE catalog…</p>
              </div>
            )}
            ...
            {error && (
              <div className="max-w-md mx-auto text-center pop-card p-8">
                <div className="w-14 h-14 mx-auto mb-4 rounded-2xl bg-destructive/10 text-destructive border-2 border-foreground/80 flex items-center justify-center shadow-pop">
                  <Bug className="w-6 h-6" strokeWidth={2.5} />
                </div>
                <h3 className="font-heading font-bold text-xl mb-2">Couldn't load the catalog</h3>
                <p className="text-sm text-muted-foreground">
                  The Android RE list couldn't be fetched right now. Check your connection and try again shortly.
                </p>
              </div>
            )}
            ...
            {!loading && !error && Object.keys(filtered).length === 0 && (
              <div className="max-w-md mx-auto text-center pop-card p-8">
                ...
                <h3 className="font-heading font-bold text-xl mb-2">No resources found</h3>
